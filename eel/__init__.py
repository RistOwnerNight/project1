...
import sys
from importlib import resources  # <<< ИЗМЕНЕНИЕ 1: Новый импорт
import socket

# <<< ИЗМЕНЕНИЕ 2: Новый метод получения пути и чтения файла >>>
with resources.as_file(resources.files('eel').joinpath('eel.js')) as path:
    _eel_js = open(path, encoding='utf-8').read()
_websockets = []
_call_return_values = {}
